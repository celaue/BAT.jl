...
@inline function checked_logdensityof(density::AbstractDensity, v::Any)
    check_variate(density, v)
    v_matched = match_variate(density, v)

    logval = try
        logdensityof(density, v_matched)
    catch err
        @rethrow_logged DensityEvalException(logdensityof, density, v_matched, err)
    end

    _check_density_logval(density, v_matched, logval)
...
ZygoteRules.@adjoint checked_logdensityof(density::AbstractDensity, v::Any) = begin
    ...
    v_matched = match_variate(density, v)
    logval, back = try
        ZygoteRules.pullback(logdensityof(density), v_matched)
    catch err
        @rethrow_logged DensityEvalException(logdensityof, density, v_matched, err)
    end
    _check_density_logval(density, v_matched, logval)
...
@inline function density_valtype(density::AbstractDensity, v::Any)
    ...
    promote_type(T, default_output_numtype(density))
end
